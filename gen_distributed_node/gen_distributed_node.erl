...
init([Module, Index, Data]) ->
    ...
    logger:debug("Starting node ~p", [Index]),
    {ok, LoopState}.
...
handle_call(Request, From, LoopState = #loop_state{module = Module, user_data = Data}) ->
    try
        ...
        error:undef ->
            io:format("Other handle_call: ~p~n", [Request]),
            {reply, ok, LoopState}
    end.
...
handle_cast({send_data, Data}, LoopState = #loop_state{user_data = UserData, module = Module, neighbour_pids = NeighbourPids}) ->
    {UpdatedData, UpdatedUserState} = try
        ...
        error:undef ->
            io:format("Other intercept_send_data: ~p", [Data]),
            {Data, UserData}
    end,
    lists:foreach(
        fun(Pid) ->
...
handle_cast({request_data, Request}, LoopState = #loop_state{user_data = UserData, module = Module, neighbour_pids = NeighbourPids, request_data_struct = ReqDataStruct}) ->
    ...
    case maps:is_key(Request, ReqDataStruct) of
        true ->
            io:format("Request ~p is already in flight, cannot send duplicate request", [Request]),
            {noreply, LoopState};
        false ->
            {UpdatedRequest, UpdatedUserState} = try
                ...
                error:undef ->
                    io:format("Other intercept_request_data: ~p", [Request]),
                    {Request, UserData}
            end,
...
handle_cast({response_data, Request, Response, From}, LoopState = #loop_state{user_data = UserData, module = Module, request_data_struct = ReqDataStruct}) ->
    {UpdatedResponseStruct, ReturnUserData} = case maps:find(Request, ReqDataStruct) of
        ...
        error ->
            io:format("Response ~p to request ~p recieved that waas never sent", [Response, Request]),
...
handle_cast(Msg, LoopState = #loop_state{module = Module, user_data = UserData}) ->
    try
        ...
        error:undef ->
            io:format("Other handle_cast: ~p~n", [Msg]),
            {noreply, LoopState}
    end.
...
handle_info(Info, LoopState = #loop_state{module = Module, user_data = UserData}) ->
    try
        ...
        error:undef ->
            io:format("Other handle_info: ~p~n", [Info]),
            {noreply, LoopState}
    end.

-spec terminate(any(), loop_state()) -> ok.
terminate(Reason, #loop_state{module = Module, user_data = UserData}) ->
    try
